---
name: code-quality
description: Use when evaluating or improving code quality, including maintainability, abstractions, design principles and patterns, refactoring, code smells, test design, and architecture. Also handles explicit "thermonuclear" or "extremely strict structural review" requests via a pressure overlay on the full review.
---
...
| Thermo pressure | User explicitly says "thermonuclear", "thermo-nuclear", or "extremely strict structural review" | Run the full review first, then `./workflow/thermo-pressure.md` |

Thermo pressure is an overlay on the full review, not a fourth mode: the target gets the full review's intake, matrix, self-verification, and confirmation stop, then the pressure pass. Fast and Analysis are never combined with it.
