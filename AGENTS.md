...
- Three classes of skills:
  - **Original**: purely self-created. Lives in `skills/<category>/<skill>/`.
  - **Fork**: a modified copy of a third-party skill. The installable copy lives in `skills/<category>/<skill>/`; a pristine upstream snapshot lives in `registry/upstream/<skill>/`. See [Forks](#forks).
  - **Third-party**: installed unmodified via `npx skills`. Never stored in this repo; only tracked in the registry.
- Keep installable skill content under `skills/workflows/` and `skills/coding/` only. Everything in those folders is copied verbatim into projects on install, so never put management metadata there.
...
- **Installable, modified copy**: `skills/<category>/<skill>/SKILL.md` — clean, no upstream files.
...
`registry/third-party.md` (human) and `registry/inventory.json` (machine) are GENERATED by `skills/workflows/manage-skills/scripts/sync-registry.mjs`. Never hand-edit them.
